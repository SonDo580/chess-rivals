...
import { Board, CastlingRight, CastlingSide, Piece, SquarePos } from "../types";
...
const nearOpponentKing = (
  ...
  const opponentKing = getPiece(opponentColor, PieceSymbol.KING);
...
const getKingPos = (board: Board, turn: Color) => {
  for (let i = 0; i < 8; i++) {
    for (let j = 0; j < 8; j++) {
      if (board[i][j] === getPiece(turn, PieceSymbol.KING)) {
...
const updateCastlingRight = (
  ...
  produce(castlingRight, (draft) => {
    ...
    if (pieceSymbol === PieceSymbol.KING) {
      draft.q = false;
      draft.k = false;
    }

    // Remove castling right for 1 side if rook moved from initial position
    if (pieceSymbol === PieceSymbol.ROOK) {
      const initialRow = pieceColor === Color.WHITE ? 7 : 0;
